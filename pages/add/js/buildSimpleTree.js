...
function calculate_branch_xy(num_branchs, i, root, multiple,name_branch){
	...
	//旋转顺序不同，确保文字的走向；
	return {width:width_branch,id:id,  type:'branch', name: name,textpath:textpath,d:d,x:root.x+i*width_trunk* multiple, y:root.y, x2:mx, y2:my, x3:x3, y3:y3, x4:x4, y4:y4, a:a, i:i, mx:mx,my:my,cx1:cx1,cy1:cy1,cx2:cx2, cy2:cy2,cx3:cx3,cy3:cy3,dx:dx,dy:dy+9};
...
function calculate_twig_xy(parent, num_branchs, i, num_twig, j,root, multiple,name_twig){	
	...
	return {width:width_twig,d:d, id:id,type:'twig',name:name,textpath:textpath, 
			cx2:cx2, cy2:cy2,x3:x3,y3:y3,x4:x4,y4:y4, a:a4,dx:dx, dy:dy-4};
...
/*
var datas = [];	
d3.json("data/binaryTree.json", function (dataset){
	datas.push(dataset);
	var seed4 = {x: 700* multiple, y: 600* multiple, name:dataset.name}; 
...
$(window).bind('mousewheel', function(evt) {	
	...
	if( 0.25< multiple && multiple<1.25){
		multiple+=evt.originalEvent.wheelDelta/480;
	}else if(multiple == 0.25){
	...
	if(temp==multiple){return;}
	
	d3.selectAll("svg").remove(); //删除之前的svg
	svg = d3.select("#showTree")
				.append("svg")
				.attr("width", w * multiple)
				.attr("height", h * multiple);
	$("svg").draggable();	
	var seed0 = {x: 500* multiple, y: 600* multiple, name:datas[0].name}; 
	var tree0 = buildTree(datas[0], seed0, multiple);
